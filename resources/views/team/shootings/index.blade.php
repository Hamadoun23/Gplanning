    ...
    <script>
        ...
        function openDateModal(date) {
            ...
            fetch(`/api/events-by-date?date=${date}`, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
                .then(r => r.json())
                .then(data => { modalLoading.style.display = 'none'; modalContent.style.display = 'block'; displayShootings(data.shootings); })
        ...
        function displayShootings(shootings) {
            const container = document.getElementById('shootingsList');
            if (shootings.length === 0) { container.innerHTML = '<div class="empty-events">Aucun tournage prévu</div>'; return; }
            container.innerHTML = shootings.map(s => {
                let sc = 'status-pending', st = 'En attente';
                if (s.status === 'completed') { sc = 'status-completed'; st = 'Complété'; }
                else if (s.status === 'cancelled') { sc = 'status-cancelled'; st = 'Annulé'; }
                else if (s.is_overdue) { sc = 'status-overdue'; st = 'En retard'; }
                else if (s.is_upcoming) { sc = 'status-upcoming'; st = 'À venir'; }
                const ideas = s.content_ideas && s.content_ideas.length > 0 ? s.content_ideas.map(ci => ci.titre || ci).join(', ') : 'Aucune idée';
                return `<div class="event-item"><div class="event-item-info"><div class="event-item-title">📹 ${s.client_name} <span class="event-status-badge ${sc}">${st}</span></div><div class="event-item-details">${ideas}</div></div><div class="event-item-actions"><a href="${s.url}" class="btn btn-primary">Voir</a></div></div>`;
            }).join('');
        }
