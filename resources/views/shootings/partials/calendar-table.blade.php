            ...
            <tr>
                @foreach($week as $day)
                    ...
                    <td class="calendar-day-cell" 
                        ...
                        data-client-id="{{ $day['shootings']->isNotEmpty() ? $day['shootings']->pluck('client_id')->unique()->implode(',') : '' }}"
                        ...
                        <div class="calendar-day-events" style="max-height: 110px; overflow-y: auto;" onclick="event.stopPropagation();">
                            <!-- Tournages uniquement -->
                            @foreach($day['shootings'] ?? [] as $shooting)
                                @php
                                    $shootDayOfWeek = $days[\Carbon\Carbon::parse($shooting->date)->format('l')] ?? '';
                                    $hasWarning = false;
                                    if ($shooting->client) {
                                        $hasWarning = $shooting->client->isDayNotRecommended($shootDayOfWeek);
                                    }
                                    ...
                                    if ($shooting->status === 'not_realized') {
                                        $bgColor = '#6c757d';
                                        $borderColor = '#5a6268';
                                        $icon = '❌';
                                ...
                                <div class="calendar-event" 
                                     ...
                                     data-client-id="{{ $shooting->client_id }}"
                                     ...
                                     title="Tournage - {{ $shooting->client ? $shooting->client->nom_entreprise : 'N/A' }} - {{ $shooting->date->format('d/m/Y H:i') }} - {{ $shooting->status === 'completed' ? 'Complété' : ($shooting->isOverdue() ? 'En retard' : ($shooting->isUpcoming() ? 'Approche' : 'En attente')) }}">
                                    <strong>{{ $icon }} {{ $shooting->client ? $shooting->client->nom_entreprise : 'N/A' }}</strong>
                                    @if($shooting->contentIdeas->count() > 0)
                                    <br><small>{{ $shooting->contentIdeas->count() }} idée(s)</small>
                                    @endif
                                    @if($hasWarning && !$shooting->isUpcoming())
