...
@section('title', 'Détails Client - ' . $client->nom_entreprise)
...
@section('content')
    ...
    <div class="client-dashboard-header">
        <div>
            ...
            <p style="color: #666; margin: 0; font-size: 1.1rem;">Détails du client - Tableau de bord</p>
        </div>
    ...
    <div class="card">
        ...
        <div class="report-upload-section">
            <h4 style="margin-bottom: 1rem; color: #303030;">Téléverser un nouveau rapport</h4>
            <form action="{{ route('clients.upload-report', $client) }}" method="POST" enctype="multipart/form-data" class="report-upload-form">
                @csrf
                <div class="report-form-fields">
                    <div class="report-form-field">
                        <label for="report_type" class="report-form-label">
                            <svg class="report-form-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="report-select-wrapper">
                            <select id="report_type" name="report_type" class="report-select" required>
                                <option value="">Sélectionner un type</option>
                                <option value="monthly">Mensuel</option>
                                <option value="annual">Annuel</option>
                            </select>
                            <svg class="report-select-arrow" width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M2 4l4 4 4-4"></path>
                            </svg>
                        </div>
                        @error('report_type')
                    ...
                    <div class="report-form-field">
                        <label for="report_date" class="report-form-label">
                            <svg class="report-form-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                                <line x1="16" y1="2" x2="16" y2="6"></line>
                                <line x1="8" y1="2" x2="8" y2="6"></line>
                        ...
                        <input type="date" id="report_date" name="report_date" class="report-date-input" value="{{ date('Y-m-d') }}" required>
                        @error('report_date')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="report-form-field">
                        <label for="report_file" class="report-form-label">
                            <svg class="report-form-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="report-file-wrapper">
                            <input type="file" id="report_file" name="report_file" accept=".pdf" class="report-file-input" required>
                            <label for="report_file" class="report-file-label">
                                <span class="report-file-text">Choisir un fichier PDF</span>
                                <span class="report-file-button">Parcourir</span>
                            </label>
                        </div>
                        @error('report_file')
                            <span class="error-message">{{ $message }}</span>
                ...
                <button type="submit" class="btn btn-primary report-upload-btn">
        ...
        <div class="reports-list-section" style="margin-top: 2rem; padding-top: 2rem; border-top: 2px solid #f0f0f0;">
                        ...
                        @foreach($monthlyReports as $report)
                            <div class="report-item">
                                ...
                                <div class="report-item-actions">
                                    ...
                                    @if(!$isTeamReadOnly)
                                        <form action="{{ route('clients.reports.delete', [$client, $report]) }}" method="POST" class="report-delete-form" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce rapport ?');">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        ...
        const reportFileText = document.querySelector('.report-file-text');
        
        if (reportFileInput && reportFileText) {
            reportFileInput.addEventListener('change', function(e) {
                if (e.target.files.length > 0) {
                    reportFileText.textContent = e.target.files[0].name;
                } else {
                    reportFileText.textContent = 'Choisir un fichier PDF';
                }
            });
        }
