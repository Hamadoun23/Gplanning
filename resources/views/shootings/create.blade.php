    ...
    <div class="publication-create-container" data-gsap="fadeIn">
        ...
        <div class="create-hero" data-gsap="fadeInUp">
            ...
            <div class="hero-content">
                <h1 class="hero-title">Nouveau tournage</h1>
                <p class="hero-subtitle">Créez un nouveau tournage pour votre planning</p>
            </div>
        </div>
        ...
        <div class="form-card-modern" data-gsap="fadeInUp">
            ...
            <form action="{{ route('shootings.store') }}" method="POST" id="shooting-form" class="publication-form-modern" autocomplete="one-time-code">
                @csrf
                ...
                <div class="form-field-modern" data-gsap="fadeInUp">
                    ...
                    <div class="input-wrapper-modern">
                        <input type="datetime-local" id="date" name="date" value="{{ old('date', $selectedDate ? \Carbon\Carbon::parse($selectedDate)->format('Y-m-d\TH:i') : '') }}" required class="input-modern" autocomplete="one-time-code" data-realtime-check data-check-type="shooting">
                ...
                <div class="form-field-modern" data-gsap="fadeInUp">
                    ...
                    <div class="textarea-wrapper-modern">
                        <textarea id="description" name="description" rows="5" class="textarea-modern" autocomplete="one-time-code" placeholder="Décrivez le tournage (optionnel)...">{{ old('description') }}</textarea>
                ...
                <div class="form-field-modern" data-gsap="fadeInUp">
                        ...
                        <select id="content_idea_id" name="content_idea_id" required class="select-modern" autocomplete="one-time-code">
                            ...
                            @if($contentIdeas->count() > 0)
                                @foreach($contentIdeas as $idea)
                                    <option value="{{ $idea->id }}" {{ old('content_idea_id') == $idea->id ? 'selected' : '' }} data-type="{{ $idea->type }}">{{ $idea->titre }} ({{ $idea->type }})</option>
                                @endforeach
                            @endif
                        </select>
                        <div class="select-arrow">
                    ...
                    <div class="form-help-text">
                        <a href="{{ route('content-ideas.create', ['return_to' => route('shootings.create', ['client_id' => $selectedClient, 'date' => $selectedDate])]) }}" class="help-link">
                ...
                <div class="form-actions-modern" data-gsap="fadeInUp">
                    <button type="submit" name="action" value="create" class="btn-primary-modern" id="submit-btn">
                        <span class="btn-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <polyline points="20 6 9 17 4 12"></polyline>
                            </svg>
                        </span>
                        <span class="btn-text">Créer le tournage</span>
                    </button>
            ...
            if (form && submitBtn) {
                form.addEventListener('submit', function(e) {
                    ...
                    submitBtn.innerHTML = `
                        ...
                        <span class="btn-text">Création en cours...</span>
                    `;
