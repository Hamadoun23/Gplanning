...
@section('content')
    ...
    <style>
        ...
        .publication-status-badge-large {
            display: inline-flex;
            align-items: center;
            gap: 0.5rem;
        ...
        .publication-info-item:hover {
            background: #f0f0f0;
            transform: translateX(4px);
        }
        
        .publication-info-item:last-child {
            margin-bottom: 0;
        }
        ...
        .publication-status-section h2::before {
            content: '';
            width: 4px;
            height: 24px;
            background: linear-gradient(135deg, #FF6A3A 0%, #e55a2a 100%);
        ...
        .publication-alert-modern.alert-danger {
            background: #fff5f5;
            border-left-color: #dc3545;
        }
        
        .publication-alert-modern.alert-warning {
            background: #fffbf0;
            border-left-color: #ffc107;
        }
        
        .publication-alert-modern.alert-success {
            background: #f0fff4;
            border-left-color: #28a745;
        }
        
        .publication-alert-modern.alert-info {
            background: #f0f9ff;
            border-left-color: #17a2b8;
        }
        
        .publication-alert-icon {
            font-size: 1.5rem;
            flex-shrink: 0;
        }
        
        .publication-alert-content {
            flex: 1;
        }
        
        .publication-alert-title {
            font-weight: 700;
            font-size: 1rem;
            margin-bottom: 0.25rem;
        }
        ...
        .publication-modal-header p {
            color: #6c757d;
            margin: 0;
        }
        
        .publication-modal-form-group {
            margin-bottom: 1.5rem;
        }
        
        .publication-modal-form-group label {
            display: block;
            font-size: 0.85rem;
            font-weight: 700;
            text-transform: uppercase;
        ...
        .publication-modal-form-group textarea,
        .publication-modal-form-group input[type="date"] {
            width: 100%;
        ...
        .publication-modal-form-group textarea:focus,
        .publication-modal-form-group input[type="date"]:focus {
            outline: none;
            border-color: #FF6A3A;
        ...
        .publication-modal-btn-secondary {
            background: #f8f9fa;
            color: #495057;
        }
        
        .publication-modal-btn-secondary:hover {
            background: #e9ecef;
        }
        
        .publication-modal-btn-primary {
            background: linear-gradient(135deg, #FF6A3A 0%, #e55a2a 100%);
            color: white;
            box-shadow: 0 4px 12px rgba(255, 106, 58, 0.3);
        }
        
        .publication-modal-btn-primary:hover {
            transform: translateY(-2px);
    ...
    <div class="publication-page" data-gsap="fadeIn">
        <!-- Alertes modernes -->
        @if($publication->isOverdue())
            <div class="publication-alert-modern alert-danger" data-gsap="fadeInUp">
                <span class="publication-alert-icon">🚨</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">En retard</div>
                    <div class="publication-alert-text">Cette publication était prévue le {{ $publication->date->format('d/m/Y H:i') }} et n'a pas encore été complétée.</div>
                </div>
        ...
        @elseif($publication->isUpcoming())
            <div class="publication-alert-modern alert-warning" data-gsap="fadeInUp">
                <span class="publication-alert-icon">⏰</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Approche</div>
                    <div class="publication-alert-text">Cette publication est prévue dans {{ now()->diffInDays($publication->date, false) }} jour(s) ({{ $publication->date->format('d/m/Y H:i') }}).</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @elseif($publication->isCompleted())
            <div class="publication-alert-modern alert-success" data-gsap="fadeInUp">
                <span class="publication-alert-icon">✅</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Complétée</div>
                    <div class="publication-alert-text">Cette publication a été marquée comme complétée.</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @elseif($publication->status === 'not_realized')
            <div class="publication-alert-modern alert-danger" data-gsap="fadeInUp">
                <span class="publication-alert-icon">❌</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Non réalisée</div>
                    <div class="publication-alert-text">Cette publication n'a pas pu être réalisée.@if($publication->status_reason) <br><em style="opacity: 0.8;">{{ $publication->status_reason }}</em>@endif</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @elseif($publication->status === 'cancelled')
            <div class="publication-alert-modern alert-danger" data-gsap="fadeInUp">
                <span class="publication-alert-icon">🚫</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Annulée</div>
                    <div class="publication-alert-text">Cette publication a été annulée.@if($publication->status_reason) <br><em style="opacity: 0.8;">{{ $publication->status_reason }}</em>@endif</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @elseif($publication->status === 'rescheduled')
            <div class="publication-alert-modern alert-info" data-gsap="fadeInUp">
                <span class="publication-alert-icon">📅</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Reprogrammée</div>
                    <div class="publication-alert-text">Cette publication a été reprogrammée.@if($publication->status_reason) <br><em style="opacity: 0.8;">{{ $publication->status_reason }}</em>@endif</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @endif
        ...
        @if($hasWarning)
            <div class="publication-alert-modern alert-warning" data-gsap="fadeInUp">
                <span class="publication-alert-icon">⚠️</span>
                <div class="publication-alert-content">
                    <div class="publication-alert-title">Avertissement</div>
                    <div class="publication-alert-text">Ce jour ({{ ucfirst($dayOfWeek) }}) est non recommandé pour la publication pour ce client.</div>
                </div>
                <button type="button" onclick="this.parentElement.remove()" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; opacity: 0.6; color: inherit;">×</button>
            </div>
        @endif
        ...
        <!-- Hero Section -->
        <div class="publication-hero" data-gsap="fadeInUp">
            <div class="publication-hero-header">
                <div class="publication-hero-title">
                    <h1>Publication</h1>
                    <div class="publication-hero-meta">
                        <div class="publication-meta-item">
                            ...
                            <span>{{ $publication->date->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="publication-meta-item">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            <span>{{ $publication->client->nom_entreprise }}</span>
                        </div>
                        <div class="publication-status-badge-large">
                            <span>{{ $currentStatus['icon'] }}</span>
                            <span>{{ $currentStatus['label'] }}</span>
                        </div>
                    </div>
                </div>
                <div class="publication-actions-header">
                    @if(!$isTeamReadOnly)
                        <a href="{{ route('publications.edit', $publication) }}" class="publication-action-btn">
                    ...
                    @endif
                    <a href="{{ route('dashboard', ['month' => $publication->date->month, 'year' => $publication->date->year]) }}" class="publication-action-btn">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="publication-content-grid">
            <!-- Carte principale -->
            <div class="publication-main-card" data-gsap="fadeInUp">
                ...
                <div class="publication-info-item">
                    ...
                    <div class="publication-info-content">
                        <div class="publication-info-label">Client</div>
                        <div class="publication-info-value">{{ $publication->client->nom_entreprise }}</div>
                    </div>
                </div>
                ...
                <div class="publication-info-item">
                    <div class="publication-info-icon">📅</div>
                    <div class="publication-info-content">
                        <div class="publication-info-label">Date et heure de publication</div>
                        <div class="publication-info-value">{{ $publication->date->format('d/m/Y H:i') }}</div>
                    </div>
                </div>
                ...
                <div class="publication-info-item">
                    <div class="publication-info-icon">💡</div>
                    <div class="publication-info-content">
                        <div class="publication-info-label">Idée de contenu</div>
                        <div class="publication-info-value">{{ $publication->contentIdea->titre }}</div>
                ...
                <div class="publication-info-item">
                    <div class="publication-info-icon">📝</div>
                    <div class="publication-info-content">
                        <div class="publication-info-label">Description</div>
                        <div class="publication-info-value" style="font-weight: 400; line-height: 1.6;">{{ $publication->description }}</div>
                    </div>
                </div>
                @endif
                
                @if($publication->status_reason)
                <div class="publication-info-item" style="background: #fff3cd; border-left: 4px solid #ffc107;">
                    <div class="publication-info-icon">ℹ️</div>
                    <div class="publication-info-content">
                        <div class="publication-info-label">Raison du statut</div>
                        <div class="publication-info-value" style="font-weight: 400; line-height: 1.6;">{{ $publication->status_reason }}</div>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="publication-sidebar">
                @if(!$isTeamReadOnly)
                <div class="publication-sidebar-card" data-gsap="fadeInUp">
                    <h3>Changer le statut</h3>
                    <div class="status-buttons-grid">
                        @php
                            $month = request()->get('return_month', $publication->date->month);
                            $year = request()->get('return_year', $publication->date->year);
                        @endphp
                        <button type="button" onclick="changeStatus('pending', '{{ $month }}', '{{ $year }}')" 
                                class="status-btn-modern status-pending {{ $publication->status === 'pending' ? 'active' : '' }}">
                        ...
                        <button type="button" onclick="changeStatus('completed', '{{ $month }}', '{{ $year }}')" 
                                class="status-btn-modern status-completed {{ $publication->status === 'completed' ? 'active' : '' }}">
                            <span class="status-btn-icon">✅</span>
                            <span class="status-btn-text">Complétée</span>
                        </button>
                        <button type="button" onclick="changeStatus('not_realized', '{{ $month }}', '{{ $year }}')" 
                                class="status-btn-modern status-not-realized {{ $publication->status === 'not_realized' ? 'active' : '' }}">
                            <span class="status-btn-icon">❌</span>
                            <span class="status-btn-text">Non réalisé</span>
                        </button>
                        <button type="button" onclick="changeStatus('cancelled', '{{ $month }}', '{{ $year }}')" 
                                class="status-btn-modern status-cancelled {{ $publication->status === 'cancelled' ? 'active' : '' }}">
                            <span class="status-btn-icon">🚫</span>
                            <span class="status-btn-text">Annulé</span>
                        </button>
                        <button type="button" onclick="changeStatus('rescheduled', '{{ $month }}', '{{ $year }}')" 
                                class="status-btn-modern status-rescheduled {{ $publication->status === 'rescheduled' ? 'active' : '' }}">
                            <span class="status-btn-icon">📅</span>
        ...
        <div id="statusReasonModal" class="publication-modal-modern">
            <div class="publication-modal-content">
                <div class="publication-modal-header">
                    <h3>Description obligatoire</h3>
                    <p>Veuillez expliquer pourquoi cette publication est <strong id="statusReasonLabel"></strong> :</p>
                </div>
                <form id="statusReasonForm" method="POST">
                    ...
                    <div class="publication-modal-actions">
                        <button type="button" onclick="closeStatusReasonModal()" class="publication-modal-btn publication-modal-btn-secondary">Annuler</button>
                        <button type="submit" class="publication-modal-btn publication-modal-btn-primary">Confirmer</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function changeStatus(status, month, year) {
            ...
            if (statusesRequiringReason.includes(status)) {
                ...
                gsap.fromTo('.publication-modal-content', 
        ...
        document.getElementById('statusReasonForm').addEventListener('submit', function(e) {
            ...
            if (status === 'rescheduled' && (!dateInput.value || dateInput.value.trim() === '')) {
                e.preventDefault();
                alert('Veuillez sélectionner une nouvelle date pour reprogrammer la publication.');
                dateInput.focus();
                return false;
            }
        });
        ...
        function closeStatusReasonModal() {
            const modal = document.getElementById('statusReasonModal');
            gsap.to('.publication-modal-content', {
                opacity: 0,
                scale: 0.9,
                y: -20,
                duration: 0.2,
